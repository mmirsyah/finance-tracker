...
BEGIN
    ...
    -- We use the p_current_month_anchor to find the start of the current period first
    -- Then go back one day to be safely in the previous period
    prev_period_start_date := public.get_custom_period_start( (public.get_custom_period_start(p_current_month_anchor, v_start_day) - INTERVAL '1 day'), v_start_day);
    prev_period_end_date := (prev_period_start_date + INTERVAL '1 month' - INTERVAL '1 day')::DATE;
    
    -- The budget assignment is anchored to the 1st of the month the period starts in
        ...
        IF rec.is_rollover THEN
            ...
            SELECT
                GREATEST(0,
                    ...
                    -
                    -- Last month's spent amount (using the correct date range)
                    COALESCE((
                        SELECT SUM(amount)
