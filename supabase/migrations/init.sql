...
CREATE OR REPLACE FUNCTION "public"."update_rollover_for_period"("p_household_id" "uuid", "p_current_month_ref_date" "date") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    v_start_day INT;
...
BEGIN
    ...
    -- 2. Determine the TARGET period's actual start date
    target_period_start_date := public.get_custom_period_start(p_current_month_ref_date, v_start_day);

    -- 3. Determine the PREVIOUS period's actual start and end dates
    ...
    FOR rec IN
        ...
        JOIN public.categories c ON ba.category_id = c.id
        WHERE ba.household_id = p_household_id
          AND ba.month = prev_period_start_date
    ...
    LOOP
        ...
        -- Calculate the exact rollover amount (can be negative)
        v_rollover_amount := v_assigned_amount - v_spent_amount;

        -- 6. Always UPSERT the calculated rollover into the TARGET month's budget
...
ALTER FUNCTION "public"."update_rollover_for_period"("p_household_id" "uuid", "p_current_month_ref_date" "date") OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."budget_assignments" (
    ...
    "rollover_amount" numeric DEFAULT 0 NOT NULL
);
...
GRANT ALL ON FUNCTION "public"."update_rollover_for_period"("p_household_id" "uuid", "p_current_month_ref_date" "date") TO "anon";
GRANT ALL ON FUNCTION "public"."update_rollover_for_period"("p_household_id" "uuid", "p_current_month_ref_date" "date") TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_rollover_for_period"("p_household_id" "uuid", "p_current_month_ref_date" "date") TO "service_role";
