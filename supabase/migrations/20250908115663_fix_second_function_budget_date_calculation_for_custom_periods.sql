-- Memperbaiki perhitungan tanggal untuk mengambil anggaran periode sebelumnya pada fungsi kedua
-- Menghitung tanggal anggaran berdasarkan tanggal awal periode kustom sebelumnya
...
CREATE OR REPLACE FUNCTION "public"."get_category_spending_history"("p_household_id" "uuid", "p_category_id" bigint, "p_reference_date" "date", "p_period_start_day" integer) 
RETURNS TABLE("spent_last_period" numeric, "period_average" numeric, "last_month_budget" numeric, "period_breakdown" "jsonb")
    LANGUAGE "plpgsql"
    AS $$
DECLARE
...
BEGIN
    -- Perbaikan: Memastikan tipe data DATE digunakan
    -- Menghitung tanggal periode dengan benar
    v_last_period_start := public.get_custom_period_start((p_reference_date - INTERVAL '1 day')::DATE, v_start_day);
    v_last_period_end := v_last_period_start + INTERVAL '1 month' - INTERVAL '1 day';
    
    -- Perbaikan: Menghitung tanggal anggaran periode sebelumnya dengan benar
    -- Kita menggunakan tanggal awal periode kustom sebelumnya, bukan tanggal 1
    v_last_month_budget_date := v_last_period_start;
    ...
    last_month_budget AS (
        ...
        -- Perbaikan: Menggunakan v_last_month_budget_date yang merupakan tanggal awal periode kustom
