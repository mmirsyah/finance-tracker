...
import { Plus, Trash2, ArrowLeft, Save, Copy, Loader2 } from 'lucide-react';
...
export default function BulkAddView() {
    ...
    const handleRowChange = (index: number, field: keyof Omit<BulkRow, 'id'>, value: string) => {
        ...
        if (field === 'type') {
            ...
            rowToUpdate.category_id = ''; // Reset kategori saat tipe berubah
        } else {
    ...
    const handleCopyDown = (index: number, field: keyof Omit<BulkRow, 'id'>) => {
        if (index >= rows.length - 1) return; // Tidak melakukan apa-apa jika baris terakhir
        
        const valueToCopy = rows[index][field];
        const newRows = [...rows];
        ...
        setRows(newRows);
        toast.info(`'${field.replace('_',' ')}' disalin ke semua baris di bawah.`);
    };
                ...
                <div className="overflow-x-auto">
                    <Table>
                        ...
                        <TableBody>
                            {rows.map((row, index) => (
                                <TableRow key={row.id}>
                                    <TableCell><Input type="date" value={row.date} onChange={(e) => handleRowChange(index, 'date', e.target.value)} /></TableCell>
                                    <TableCell>
                                    ...
                                    <TableCell className="text-right">
                                        <div className="flex items-center gap-1">
                                          ...
                                          <Button variant="ghost" size="icon" title="Salin tanggal ke bawah" onClick={() => handleCopyDown(index, 'date')}><Copy className="h-4 w-4"/></Button>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ))}
