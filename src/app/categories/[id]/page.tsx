...
import { format, getYear, getQuarter, startOfMonth, isWithinInterval, startOfQuarter, endOfQuarter, startOfYear, endOfYear, type Interval } from 'date-fns';

const formatCurrency = (value: number) => {
    return new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(value);
};

const formatDateRange = (date: Date | number | string) => {
                ...
                <div className="flex flex-wrap justify-between items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="p-1 bg-gray-200 rounded-md flex items-center gap-1">
                            <FilterButton view="monthly" label="Monthly" /><FilterButton view="quarterly" label="Quarterly" /><FilterButton view="yearly" label="Yearly" />
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {processedData.displayedTransactions.map((t) => (
                            <tr key={t.id}>
                                ...
                                <td className="px-6 py-4 text-sm text-gray-800 font-medium">{t.accounts?.name || t.to_account?.name || 'N/A'}</td>
